...
$on_mod(Loaded) {
	...
	if (std::filesystem::exists(jumpscareAudio) && Utils::acceptableAudioFileExtension(jumpscareAudio.extension())) Manager::get()->system->createSound(geode::utils::string::pathToString(jumpscareAudio).c_str(), FMOD_DEFAULT, nullptr, &Manager::get()->sound);
	...
	listenForSettingChanges<std::filesystem::path>("jumpscareImage", [](const std::filesystem::path& path) {
		Utils::removeUNJUS();
		if (Utils::acceptableImageFileExtension(path.extension())) Utils::addUNJUS(path);
	});
